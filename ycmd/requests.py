...
def GetCompletions( request_data ):
  ...
  return BuildCompletionResponse( completions if completions else [],
...
def FilterAndSortCandidates( request_data ):
  ...
  return FilterAndSortCandidatesWrap(
...
def GetReady( subserver=None, include_subservers=False ):
...
def DefinedSubcommands( request_data ):
...
def LoadExtraConfFile( request_data ):
  ...
  request_data = RequestWrap( request_data, validate = False )
...
def GetLoadedFiletypeCompleters():
  return _server_state.GetLoadedFiletypeCompleters()
